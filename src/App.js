...
function App() {
	...
	const style_main = toggle ? { backgroundColor: "#aaaaaa8f" } : {};
	const style_graph = !toggle ? { backgroundColor: "#aaaaaa8f" } : {};
	...
	react.useEffect(() => {
		tracker.forEach((value, index) => {
			const dieCountElement = document.getElementById(`die-count-${index + 2}`);
			if (dieCountElement) {
				dieCountElement.textContent = value;
			}
		})
	}, [tracker]);
							...
							<div className="die-column">
								<div className="die-container">
									<table style={{ border: "1px solid black" }}>
										...
										{dieElements.map((dieElement, index) => (
											<tr key={index}>
												<td>{dieElement}</td>
												<td id={`die-count-${index + 2}`}>{tracker[dieElement - 2]}</td>
											</tr>
										))}
									</table>

									{/* Undo button */}
									<button className="undo-button" onClick={() => changeDieValue(dieHistory.length > 0 ? dieHistory[dieHistory.length - 1] : null, false)}><img src={undoIcon} alt="Undo" style={{ width: "1em", height: "1em" }} /></button>


									{/* Die history display */}
									<div className="die-history-container">
										<p style={{ backgroundColor: '#ffffff', padding: '0.5em', border: '1px solid black' }}>Dice History: {dieHistory.toReversed().map((value, index) => {
											// color code the die rolls based on their value
											let colorHex = '#000000';
											if (value === 2 || value === 12) colorHex = '#f032e6';
											if (value === 3 || value === 11) colorHex = '#f58231';
											if (value === 4 || value === 10) colorHex = '#e6194B';
											if (value === 5 || value === 9) colorHex = '#4363d8';
											if (value === 6 || value === 8) colorHex = ''
											console.log(colorHex)
											return <span key={index} style={{ color: colorHex, fontWeight: 'bold' }}>
												{value + ' '}
											</span>
										}
